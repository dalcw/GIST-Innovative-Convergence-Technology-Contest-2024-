...
# dataset2 = pd.read_csv("./2023_train.csv").values[:1072]  # 일부 데이터 버리기 - 결측치 너무 많음
# total_dataset = np.concatenate((dataset1, dataset2))

norm_dataset, maximum, minimum = minmax(dataset1)
with open("elec_min.pickle","wb") as f:
    pickle.dump(minimum, f)

with open("elec_max.pickle", "wb") as f:
...
for i in range(62, 63):
    ...
    # if i == 10 or i == 11:
    #     X = np.array(np.concatenate((minmax(dataset1, minimum, maximum)[:, 2:4], minmax(dataset1, minimum, maximum)[:, 5:12]), axis=1))
    # else:
    X = np.array(np.concatenate((norm_dataset[:, 2:4], norm_dataset[:, 5:12]), axis=1))
    y = np.array(norm_dataset[:, i])
