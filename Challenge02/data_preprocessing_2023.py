...
# 파일명
file_list = os.listdir("../../2023/태양광일보")[:-1]
file_list.sort()
...
for i in range(len(file_list)):
    df = pd.read_excel("../../2023/태양광일보/" + file_list[i])
    environment = df.iloc[5:-1, 1:5]
    solar_power_per_hour = df.iloc[5:-1, 6:-3:2]
...
full_df = pd.concat((weather_data.iloc[48:-24, 3:].reset_index(drop=True), total_df.reset_index(drop=True)), axis=1)
full_df.columns = col
full_df.clip(lower=0) # 발전량은 음수가 될 수 없기에 음수인 경우 0으로 처리
